...
async function fetchLeetCodeActivity(username: string) {
  ...
  try {
    ...
    return { days, streak: calendar.streak ?? 0, totalActiveDays: calendar.totalActiveDays ?? 0 };
  } catch {
    return null;
  }
}

async function fetchGitHubContributionTotal(token: string, username: string, createdAt: string, end: Date) {
  ...
  const totals = await Promise.all(ranges.map(async (range) => {
    const response = await fetch("https://api.github.com/graphql", {
      method: "POST",
      headers: { authorization: `Bearer ${token}`, "content-type": "application/json", "x-github-api-version": githubApiVersion },
      body: JSON.stringify({ query, variables: { login: username, from: range.from.toISOString(), to: range.to.toISOString() } }),
      next: { revalidate: 900 },
    });
...
export async function GET() {
  ...
  const [owner = "NoobSambit", repository = "sambit-pradhan"] = (process.env.GITHUB_REPOSITORY || "NoobSambit/sambit-pradhan").split("/");

  if (!token) {
    return NextResponse.json({ error: "GitHub data is not configured." }, { status: 503 });
  }
  ...
  try {
    const [graphqlResponse, commitsResponse, leetCode] = await Promise.all([
      fetch("https://api.github.com/graphql", {
        ...
        body: JSON.stringify({ query: contributionQuery, variables: { login: username, from: start.toISOString(), to: end.toISOString(), owner, name: repository } }),
        next: { revalidate: 300 },
      }),
      fetch(`https://api.github.com/repos/${owner}/${repository}/commits?per_page=9`, {
        headers: { authorization: `Bearer ${token}`, accept: "application/vnd.github+json", "x-github-api-version": githubApiVersion },
        next: { revalidate: 300 },
      }),
    ...
    if (!graphqlResponse.ok || !commitsResponse.ok) {
      return NextResponse.json({ error: "GitHub did not return portfolio data." }, { status: 502 });
    }

    const graphql = await graphqlResponse.json();
    if (graphql.errors?.length) return NextResponse.json({ error: "GitHub contribution data is unavailable." }, { status: 502 });

    const github = graphql.data;
    const contributionDays = (github.user.contributionsCollection.contributionCalendar.weeks as { contributionDays: ContributionDay[] }[])
      .flatMap((week) => week.contributionDays)
      .slice(-displayedActivityDays)
      .map((day) => ({ date: day.date, count: day.contributionCount }));
    const allTimeContributions = await fetchGitHubContributionTotal(token, username, github.user.createdAt, end);
    const commits = (await commitsResponse.json()).map((commit: { sha: string; commit: { message: string; author: { date: string } } }) => ({
    ...
    return NextResponse.json({ error: "Live portfolio data could not be loaded." }, { status: 502 });
  }
}
